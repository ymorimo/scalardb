...
(* For instance, if the parameters are as follows,
   - ops:
     - [type |-> "insert", tx_id |-> 1, prev_tx_id |-> Null]
     - [type |-> "update", tx_id |-> 2, prev_tx_id |-> 2]
     - [type |-> "delete", tx_id |-> 3, prev_tx_id |-> 2]
     - [type |-> "insert", tx_id |-> 9, prev_tx_id |-> Null]
   - candidate_separate_ops:
     - ins_ops |-> {}
     - non_ins_ops |-> [t1 |-> Null, t2 |-> Null]
   - inserted_tx_ids: ["t9"]
   the following function is returned
   - ins_ops |-> [type |-> "insert", tx_id |-> 1, prev_tx_id |-> Null]
   - non_ins_ops |-> [
       t1 |-> [type |-> "update", tx_id |-> 2, prev_tx_id |-> 1],
       t2 |-> [type |-> "delete", tx_id |-> 3, prev_tx_id |-> 2]
     ]
...
variable
    stored_ops = <<
        \* Type 1
        {
            [prev_tx_id |-> Null, tx_id |-> 1, type |-> "insert"],
            [prev_tx_id |-> 1,    tx_id |-> 2, type |-> "update"],
            [prev_tx_id |-> 2,    tx_id |-> 3, type |-> "delete"],
            [prev_tx_id |-> Null, tx_id |-> 9, type |-> "insert"]
        },
        \* Type 2
        {
            [prev_tx_id |-> Null, tx_id |-> 1, type |-> "insert"],
            [prev_tx_id |-> 1,    tx_id |-> 2, type |-> "delete"],
            [prev_tx_id |-> Null, tx_id |-> 3, type |-> "insert"],
            [prev_tx_id |-> 3,    tx_id |-> 4, type |-> "delete"],
            [prev_tx_id |-> Null, tx_id |-> 9, type |-> "insert"]
        }
    >>[OperationTypeIndex],
...
define
    ...
    ExpectedState ==
        ...
        /\ LastItemOfSequence(secondary_db_record["applied_tx_ids"]) = 9
        ...
        *)
        ...
        while ~need_to_stop do
            if ~CurrentRecordExists(new_cur_record_info) /\ candidate_separate_ops["ins_ops"] /= {} then
                ...
                *)
                tmp_op := CHOOSE x \in candidate_separate_ops["ins_ops"]: \A y \in candidate_separate_ops["ins_ops"]: x["tx_id"] <= y["tx_id"];
        ...
        if ops_to_be_moved /= <<>> then
          ...
          TODO: Update this diagram.
            ...
            *)
            ...
            if fetched_cur_record["tx_id"] = Null then
                \* `version` = Null means the record doesn't exist on the secondary DB table.
                if secondary_db_record["version"] = Null then
                    secondary_db_record := [
                        tx_id |-> new_cur_record_info["tx_id"],
                        version |-> new_cur_record_info["version"],
                        deleted |-> new_cur_record_info["deleted"],
                        applied_tx_ids |-> tmp_applied_tx_ids_on_secondary_db
                    ];
                else
                    \* Retry with version comparison, considering the case the secondary DB table record was inserted by another thread.
                    if secondary_db_record["version"] <= new_cur_record_info["version"] then
                        secondary_db_record := [
                            tx_id |-> new_cur_record_info["tx_id"],
                            version |-> new_cur_record_info["version"],
                            deleted |-> new_cur_record_info["deleted"],
                        ...
                        \* The secondary DB table record is already updated with a newer state. Let's give up.
...
BuildNextOperationByMergingOperationChain(self) == /\ pc[self] = "BuildNextOperationByMergingOperationChain"
                                                   /\ IF ~need_to_stop[self]
                                                         THEN /\ IF ~CurrentRecordExists(new_cur_record_info[self]) /\ candidate_separate_ops[self]["ins_ops"] /= {}
                                                                    THEN /\ tmp_op' = [tmp_op EXCEPT ![self] = CHOOSE x \in candidate_separate_ops[self]["ins_ops"]: \A y \in candidate_separate_ops[self]["ins_ops"]: x["tx_id"] <= y["tx_id"]]
                                                                         /\ Assert(tmp_op'[self]["type"] = "insert", 
                                                                                   "Failure of assertion at line 295, column 17.")
                                                         ...
                                                         ELSE /\ IF ops_to_be_moved[self] /= <<>>
                                                                    THEN /\ pc' = [pc EXCEPT ![self] = "WriteOpsToSecondaryDbRecord"]
...
WriteOpsToSecondaryDbRecord(self) == /\ pc[self] = "WriteOpsToSecondaryDbRecord"
                                     ...
                                     /\ IF fetched_cur_record[self]["tx_id"] = Null
                                           THEN /\ IF secondary_db_record["version"] = Null
                                                      THEN /\ secondary_db_record' =                        [
                                                                                         tx_id |-> new_cur_record_info[self]["tx_id"],
                                                                                         version |-> new_cur_record_info[self]["version"],
                                                      ...
                                                      ELSE /\ IF secondary_db_record["version"] <= new_cur_record_info[self]["version"]
                                                                 THEN /\ secondary_db_record' =                        [
                                                                                                    tx_id |-> new_cur_record_info[self]["tx_id"],
                                                                                                    version |-> new_cur_record_info[self]["version"],
                                                                                                    deleted |-> new_cur_record_info[self]["deleted"],
