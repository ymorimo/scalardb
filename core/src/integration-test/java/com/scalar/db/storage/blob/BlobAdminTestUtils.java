...
public class BlobAdminTestUtils extends AdminTestUtils {
  private final BlobContainerClient client;
  private final String metadataNamespace;

  public BlobAdminTestUtils(Properties properties) {
    super(properties);
    BlobConfig config = new BlobConfig(new DatabaseConfig(properties));
    client =
    ...
    client
        .listBlobs(
            new ListBlobsOptions()
                .setPrefix(
                    BlobUtils.getObjectKey(
                        metadataNamespace, BlobAdmin.NAMESPACE_TABLE, null, null)),
            null)
        .stream()
    ...
    client
        .listBlobs(
            new ListBlobsOptions()
                .setPrefix(
                    BlobUtils.getObjectKey(
                        metadataNamespace, BlobAdmin.METADATA_TABLE, null, null)),
            null)
        .stream()
        .map(BlobItem::getName)
        .collect(Collectors.toList())
  ...
  public void corruptMetadata(String namespace, String table) throws Exception {
    client
        .getBlobClient(BlobUtils.getObjectKey(metadataNamespace, table, null, null))
