...
def main():
    ...
    try:
        ...
        with multiprocessing.Pool(processes=num_cores) as pool:
            # Create a list of tasks, each with row number and column types
            tasks = [(i, column_types) for i in range(num_rows)]
            ...
            with open(args.output_file, 'a') as f:
                for chunk_idx in range(0, len(tasks), chunk_size):
                    ...
                    results = pool.map(worker_task, chunk)
                    for row in results:
                        f.write(row + '\n')
        ...
        print(f"Successfully generated {args.output_file} with {num_rows} data rows ({size_str}) for table '{args.table_name}'.")
